# RepairCore – Laravel Repair Shop Management System

![RepairCore Logo](https://repaircore.org/images/project-logo300.png)

RepairCore is a **premium Laravel-based repair shop management system** designed to help businesses manage **jobs, customers, inventory, and invoices** in a single platform. The full source is available as a premium product at [RepairCore.org](https://repaircore.org).


## Key Features

- 🔄 Complete repair lifecycle management
- 💰 Advanced financial management with Tax System
- 📦 Parts & inventory control
- 👥 Multi-user & role management
- 📊 Rich analytics dashboard
- 📄 Professional document generation
- 📚 Localization
- 📦 CMS management
- 📦 Customer portal

## Technology Stack

- **Backend:** Laravel 12, PHP 8.2+
- **Frontend:** Livewire 3.6, Bootstrap 5.3, Alpine.js
- **Database:** MySQL/PostgreSQL
- **Build Tool:** Vite 7

## 🔧 Quick Start

This repository is **not the full source**. To access the premium version, visit [RepairCore.org](https://repaircore.org).

For demonstration purposes, you can view screenshots, workflow, and features directly on the website.

## 🚀 Learn More

- Official website: [https://repaircore.org](https://repaircore.org)
- Documentation & support: [https://repaircore.org/docs](https://repaircore.org/docs)
